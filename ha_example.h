...
class ha_example : public handler {
    ...
    u8 *record_buffer = NULL;
    u8 *key_buffer = NULL;
    uint buf_size = 0;

    // initialized when open a table
    u16 max_second_key_length;
    u16 primary_key_length;

    int pack_row(u8 *buf);
    int unpack_row(const void *from, u8 *to);
    void pack_key(uchar *key, int key_num, const uchar *record);

    int get_cur_row(int key, u8 *buf);

    // storage
    BTree *p_btree = NULL; // primary key
    BTree *s_btree = NULL;

    BTree::Iterator primary_it;
    BTree::Iterator sec_it;
    bool first_row = false; // table scan
    bool key_read = false;
...
public:
    ...
    */
     ...
     */
    uint max_supported_record_length() const { return 4096 / 2; }
    ...
    /** @brief
      ...
      support indexes.
     */
    uint max_supported_key_length() const { return 512; }
